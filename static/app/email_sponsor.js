define(function (require) {
  ...
  var emailsViewModel = function(){
      ...
      self.emlsURI = '/emailpatrocinador';
      ...
      if (typeof (param) !== 'undefined') {
          self.ajax(self.emlsURI+"?sponsor="+param,'GET').done(function(data) {
              self.sponsor(data.Sponsor);
              self.supid(param);
              for (var i = 0; i < data.Length; i++) {                        
                  self.emails.push({
                      id: ko.observable(data.Emails[i].ID),
                      email: ko.observable(data.Emails[i].Email),
                      supp: ko.observable(data.Emails[i].SponsorId),
      ...
      self.add = function(email){
          var data = {email:email.email(),sponsor:email.supp()};
